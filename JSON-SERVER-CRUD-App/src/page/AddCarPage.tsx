...
import { Car, Calendar, IndianRupee, Image as ImageIcon, PlusCircle, LayoutDashboard, Gauge, Flame, Zap, Battery } from "lucide-react";
...
export default function AddCarPage() {
  ...
  const carBrand = [ "Honda", "Toyota", "Ford", "BMW", "Mercedes-Benz", "Audi", "Hyundai", "Tata", "Mahindra", "Skoda", "Volkswagen", "Nissan", "Renault", "Jeep", "Fiat", "Volvo", "Kia", "Jaguar", "Land Rover", "Bentley", "Porsche", "McLaren" ];

  const carSegment = [ "MUV", "SUV", "Sedan", "Hatchback", "Compact Hatchback", "Coupe", "Luxury", "Convertible", "SuperCar" ];
 ...
 const validateForm = () => {
  ...
  const onHandleSubmit = async (event: any) => {
    ...
    const status = await addCar(carData);
                  ...
                  {carFuelTypes.map((fuel) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center gap-2 w-full">
                        <fuel.icon
                          className={`h-6 w-6 ${fuel.color} ${selectedFuel === fuel.value ? "scale-110" : ""} transition-transform duration-200`}
                        />
                        <span
                          className={`text-sm font-medium ${selectedFuel === fuel.value ? "text-indigo-600" : "text-slate-600"}`}
                        >
                          {fuel.label}
                        </span>
                      </div>
